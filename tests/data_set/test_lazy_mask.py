...
from learning.data_set.constructors import create_lazy_mask


class TestLazyMask(TestCase):
    def test_mask_returns_all_testing_when_percentage_is_0(self):
        mask = create_lazy_mask(percentage=0)
        for index in range(32):
            self.assertTrue(mask.in_testing_set(index))
    ...
    def test_mask_returns_all_training_when_percentage_is_1(self):
        ...
        for index in range(32):
            self.assertFalse(mask.in_testing_set(index))
            self.assertTrue(mask.in_training_set(index))

    def test_mask_is_returns_opposite_values_for_testing_and_training(self):
        mask = create_lazy_mask(0.5)
        for check in range(10):
            for index in range(100):
                self.assertNotEqual(mask.in_testing_set(index), mask.in_training_set(index))

    def test_mask_is_consistent_across_rounds(self):
        mask = create_lazy_mask(0.5)
        training_expected = [mask.in_training_set(i) for i in range(100)]
        testing_expected = [mask.in_testing_set(i) for i in range(100)]
        ...
        for check in range(10):
            ...
            for index in indices:
                ...
                self.assertEqual(testing_expected[index], mask.in_testing_set(index))
    ...
    def test_mask_is_split_correctly_50_50(self):
        ...
        indices_in_testing = [mask.in_testing_set(i) for i in range(2 ** 14)]
        ...
        self.assertAlmostEqual(0.5, sum(indices_in_testing) / 2 ** 14, delta=self._calc_deviation(2 ** 14, 0.5))
    ...
    def test_mask_is_split_correctly_30_70(self):
        ...
        self.assertAlmostEqual(0.7, sum(indices_in_testing) / 2 ** 14, delta=self._calc_deviation(2 ** 14, 0.7))
