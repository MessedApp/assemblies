...
from training.constructors import create_training_set_from_list
...
class TestCallableTrainingSet(TestCase):
    def test_training_set_with_list_of_2(self):
        s = create_training_set_from_list([1, 0])
        self.assertEqual(1, s.domain_size)
        self.assertEqual(1, next(s))
        self.assertEqual(0, next(s))
        self.assertRaises(StopIteration, next, s)

    def test_training_set_with_list_of_3_fails(self):
        self.assertRaises(DataSetSizeError, create_training_set_from_list,
                          [1, 0, 1])

    def test_training_set_with_non_boolean_values(self):
        s = create_training_set_from_list([1, 3])
        self.assertEqual(1, next(s))
    ...
    def test_training_set_with_list_of_16(self):
        ...
        s = create_training_set_from_list(expected)
    ...
    def test_training_set_with_full_noise(self):
        ...
        s = create_training_set_from_list(expected_not_noisy, noise_probability=1)
        for i, value in enumerate(s):
    ...
    def test_training_set_with_noise(self):
        ...
        s = create_training_set_from_list(expected_not_noisy, noise_probability=0.5)
